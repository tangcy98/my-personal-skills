...
description: Guide for creating Claude Code skills following Anthropic's
  official best practices. Use when user wants to create a new skill, build a
  skill, write SKILL.md, update an existing skill, or needs skill creation
  guidelines. Provides structure, frontmatter fields, naming conventions, and
  new features like dynamic context injection and subagent execution.
---
...
This project uses a unified `skills/` directory at the repo root, symlinked into tool-specific config directories:

```
<project-root>/
├── skills/                  # Canonical location for all skills
│   ├── code-review/
│   ├── creating-skills/
│   ├── dev/
│   └── git-commit/
├── .opencode/
│   └── skills -> ../skills  # Symlink for OpenCode
└── .codewiz/
    └── skills -> ../skills  # Symlink for Codewiz
```

This way a single set of skill definitions is shared across multiple AI coding tools.

## Quick start

```bash
# 1. Create skill directory (in the project's unified skills/ dir)
...
# 4. Ensure symlinks exist (one-time setup per tool)
# ln -s ../skills .opencode/skills
# ln -s ../skills .codewiz/skills
```
...
```
<what it does>. Use when <triggers>. <capabilities>.
```
